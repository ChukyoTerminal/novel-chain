...
interface ThreadCardProps {
    thread: Thread;
    color: keyof typeof ThreadCardColorProps;
}

// ThreadCardコンポーネント
export function ThreadCard({ thread, color }: ThreadCardProps) {
    return (
        <div className="flex w-fit h-fit">
            <Card key={thread.id} className="relative w-55 h-70 p-0 overflow-hidden z-10 rounded-r-xl rounded-l-none shadow-md shadow-gray-400">
                <Button variant="ghost" size="lg" className="w-full h-full p-0">
                    <Link href={`/read/${thread.id}`} className={`w-full h-full flex flex-col p-2 ${ThreadCardColorProps[color].front_cover}`}>
            ...
            <div className={`relative z-0 h-5 w-55 top-68 right-57 ${ThreadCardColorProps[color].back_cover} rounded-r-xl shadow-xl shadow-gray-800`} />
            <div className={`relative ${ThreadCardColorProps[color].aspect_cover} h-71 w-5 top-1 right-113 rounded-sm -skew-y-[25deg] z-20 shadow-md`} />
